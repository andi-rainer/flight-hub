...
export function TemplateEditorPage({ templateId }: TemplateEditorPageProps) {
  ...
  const updateTextZone = (zone: 'description' | 'recipient' | 'voucherCode' | 'validUntil', key: string, value: number) => {
    if (!template) return
    const currentTextZones = template.text_zones || {}
    const currentZone = currentTextZones[zone] || {}
            ...
            <div className="p-6">
              <Tabs defaultValue="layout" className="w-full">
                ...
                <TabsContent value="layout" className="space-y-4 mt-6">
                  ...
                  <Card>
                    <CardHeader>
                      <CardTitle className="text-sm">Ticket-Style Options</CardTitle>
                      <CardDescription>Configure ticket-style vouchers with custom height and cut-lines</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div className="space-y-2">
                        <Label className="text-sm">Page Height %</Label>
                        <Slider
                          value={[template.page_height_percentage || 100]}
                          onValueChange={([value]) =>
                            setTemplate({ ...template, page_height_percentage: value })
                          }
                          min={30}
                          max={100}
                          step={5}
                        ...
                        <span className="text-xs text-muted-foreground">
                          {template.page_height_percentage || 100}% (40% = 2.5 vouchers per page)
                        </span>
                      </div>
                      <div className="flex items-center justify-between">
                        <Label className="text-sm">Show Cut-Line</Label>
                        <Switch
                          checked={template.show_cut_line ?? false}
                          onCheckedChange={(checked) =>
                            setTemplate({ ...template, show_cut_line: checked })
                          }
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        <Label className="text-sm">Show Recipient Name</Label>
                        <Switch
                          checked={template.show_recipient_name ?? true}
                          onCheckedChange={(checked) =>
                            setTemplate({ ...template, show_recipient_name: checked })
                          }
                        />
                      </div>
                    </CardContent>
                  ...
                  <Card>
                    <CardHeader>
                      <CardTitle className="text-sm">Text Zone Positioning</CardTitle>
                      <CardDescription>Freely position text elements on the voucher</CardDescription>
                    </CardHeader>
                      ...
                      <div className="space-y-3 pb-3 border-b">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="space-y-1">
                            ...
                            <Slider
                              value={[template.text_zones?.description?.x || 60]}
                              onValueChange={([value]) => updateTextZone('description', 'x', value)}
                              max={500}
                              step={5}
                            />
                            <span className="text-xs text-muted-foreground">
                              {template.text_zones?.description?.x || 60}px
                            </span>
                          </div>
                          <div className="space-y-1">
                            <Label className="text-xs">Y Position</Label>
                            <Slider
                              value={[template.text_zones?.description?.y || 260]}
                              onValueChange={([value]) => updateTextZone('description', 'y', value)}
                              max={700}
                              step={5}
                            />
                            <span className="text-xs text-muted-foreground">
                              {template.text_zones?.description?.y || 260}px
                            </span>
                          </div>
                          <div className="space-y-1">
                            <Label className="text-xs">Width</Label>
                            <Slider
                              value={[template.text_zones?.description?.width || 475]}
                              onValueChange={([value]) => updateTextZone('description', 'width', value)}
                              min={100}
                              max={500}
                              step={5}
                            />
                            <span className="text-xs text-muted-foreground">
                              {template.text_zones?.description?.width || 475}px
                            </span>
                          </div>
                          <div className="space-y-1">
                            <Label className="text-xs">Height</Label>
                            <Slider
                              value={[template.text_zones?.description?.height || 100]}
                              onValueChange={([value]) => updateTextZone('description', 'height', value)}
                              min={50}
                              max={300}
                              step={5}
                            />
                            <span className="text-xs text-muted-foreground">
                              {template.text_zones?.description?.height || 100}px
                            </span>
                          </div>
                        </div>
                        <div className="space-y-1">
                          ...
                          <Slider
                            value={[template.text_zones?.description?.fontSize || 12]}
                            onValueChange={([value]) => updateTextZone('description', 'fontSize', value)}
                            min={8}
                            max={20}
                            step={1}
                          />
                          <span className="text-xs text-muted-foreground">
                            {template.text_zones?.description?.fontSize || 12}px
                          </span>
                        </div>
                      </div>
                        ...
                        <div className="space-y-3 pb-3 border-b">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div className="space-y-1">
                              <Label className="text-xs">X Position</Label>
                              <Slider
                                value={[template.text_zones?.recipient?.x || 60]}
                                onValueChange={([value]) => updateTextZone('recipient', 'x', value)}
                                max={500}
                                step={5}
                              />
                              <span className="text-xs text-muted-foreground">
                                {template.text_zones?.recipient?.x || 60}px
                              </span>
                            </div>
                            <div className="space-y-1">
                              <Label className="text-xs">Y Position</Label>
                              <Slider
                                value={[template.text_zones?.recipient?.y || 200]}
                                onValueChange={([value]) => updateTextZone('recipient', 'y', value)}
                                max={700}
                                step={5}
                              />
                              <span className="text-xs text-muted-foreground">
                                {template.text_zones?.recipient?.y || 200}px
                              </span>
                            </div>
                          </div>
                          <div className="space-y-1">
                            <Label className="text-xs">Font Size</Label>
                            <Slider
                              value={[template.text_zones?.recipient?.fontSize || 12]}
                              onValueChange={([value]) => updateTextZone('recipient', 'fontSize', value)}
                              min={8}
                              max={20}
                              step={1}
                            />
                            <span className="text-xs text-muted-foreground">
                              {template.text_zones?.recipient?.fontSize || 12}px
                            </span>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="space-y-3 pb-3 border-b">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="space-y-1">
                            <Label className="text-xs">X Position</Label>
                            <Slider
                              value={[template.text_zones?.voucherCode?.x || 60]}
                              onValueChange={([value]) => updateTextZone('voucherCode', 'x', value)}
                              max={500}
                              step={5}
                            />
                            <span className="text-xs text-muted-foreground">
                              {template.text_zones?.voucherCode?.x || 60}px
                            </span>
                          </div>
                          <div className="space-y-1">
                            <Label className="text-xs">Y Position</Label>
                            <Slider
                              value={[template.text_zones?.voucherCode?.y || 150]}
                              onValueChange={([value]) => updateTextZone('voucherCode', 'y', value)}
                              max={700}
                              step={5}
                            />
                            <span className="text-xs text-muted-foreground">
                              {template.text_zones?.voucherCode?.y || 150}px
                            </span>
                          </div>
                        </div>
                        <div className="space-y-1">
                          <Label className="text-xs">Font Size</Label>
                          <Slider
                            value={[template.text_zones?.voucherCode?.fontSize || 12]}
                            onValueChange={([value]) => updateTextZone('voucherCode', 'fontSize', value)}
                            min={8}
                            max={20}
                            step={1}
                          />
                          <span className="text-xs text-muted-foreground">
                            {template.text_zones?.voucherCode?.fontSize || 12}px
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="space-y-1">
                            <Label className="text-xs">X Position</Label>
                            <Slider
                              value={[template.text_zones?.validUntil?.x || 60]}
                              onValueChange={([value]) => updateTextZone('validUntil', 'x', value)}
                              max={500}
                              step={5}
                            />
                            <span className="text-xs text-muted-foreground">
                              {template.text_zones?.validUntil?.x || 60}px
                            </span>
                          </div>
                          <div className="space-y-1">
                            <Label className="text-xs">Y Position</Label>
                            <Slider
                              value={[template.text_zones?.validUntil?.y || 180]}
                              onValueChange={([value]) => updateTextZone('validUntil', 'y', value)}
                              max={700}
                              step={5}
                            />
                            <span className="text-xs text-muted-foreground">
                              {template.text_zones?.validUntil?.y || 180}px
                            </span>
                          </div>
                        </div>
                        <div className="space-y-1">
                          <Label className="text-xs">Font Size</Label>
                          <Slider
                            value={[template.text_zones?.validUntil?.fontSize || 12]}
                            onValueChange={([value]) => updateTextZone('validUntil', 'fontSize', value)}
                            min={8}
                            max={20}
                            step={1}
                          />
                          <span className="text-xs text-muted-foreground">
                            {template.text_zones?.validUntil?.fontSize || 12}px
                          </span>
                        </div>
                      </div>
                    </CardContent>
